...
# ==============================================================================
# DISPLAY SETTINGS (so printed tables are readable instead of wrapping/truncating)
# ==============================================================================
...
Business-oriented penalty matrix (REVISED):

Actual High:
    High   -> 0
    Medium -> 20
    Low    -> 45   <-- STILL the highest penalty in the matrix
...
The highest penalty is still HIGH -> LOW, because missing an urgent ticket
is the worst outcome. The original version of this matrix used 100 for
HIGH->LOW while only charging 5 for LOW->HIGH/LOW->MEDIUM. That asymmetry
was so extreme that every tuned model learned to avoid predicting "low"
almost entirely (observed low-class recall as low as 6% for the winning
model), since refusing to ever guess "low" was the cheapest way to avoid
the 100-point penalty. This revised matrix keeps HIGH->LOW as the costliest
single error, but narrows the gap so models are no longer rewarded for
ignoring the "low" class outright. Validated result: low-class recall rose
from ~6% to ~56% while HIGH->LOW protection was preserved.
"""
...
joblib.dump(type_winner_model, "issue_type_model.joblib")
...
joblib.dump(priority_winner_model, "priority_model.joblib")
...
# ==============================================================================
# 12. EXECUTIVE SUMMARY — one clear, plain-language block (console + text file)
# ==============================================================================
...
def format_executive_summary():
    ...
    # ---- Issue Type -------------------------------------------------------
    t = type_summary.iloc[0]
    lines.append("\nTASK: Issue Type Classification")
    lines.append("-" * 80)
    lines.append(f"  Winner model      : {t['Winner Model']}")
    lines.append(f"  Baseline model    : {t['Baseline Model']}")
    lines.append(f"  Accuracy          : {t['Accuracy']:.1%}   (baseline: {t['Baseline Accuracy']:.1%})")
    lines.append(f"  Macro F1          : {t['Macro F1']:.4f}   (baseline: {t['Baseline Macro F1']:.4f})")
    lines.append("  Per-class F1:")
    for label in sorted(df["type"].unique()):
        key = f"winner_{label}_f1"
        if key in t:
            lines.append(f"    - {label:<10s}: {t[key]:.4f}")

    # ---- Priority -----------------------------------------------------------
    p = priority_summary.iloc[0]
    lines.append("\nTASK: Priority Classification")
    lines.append("-" * 80)
    lines.append(f"  Winner model      : {p['Winner Model']}")
    lines.append(f"  Baseline model    : {p['Baseline Model']}")
    lines.append(f"  Accuracy          : {p['Accuracy']:.1%}   (baseline: {p['Baseline Accuracy']:.1%})")
    lines.append(f"  Macro F1          : {p['Macro F1']:.4f}   (baseline: {p['Baseline Macro F1']:.4f})")
    lines.append(f"  Cost per ticket   : {p['Cost Per Ticket']:.2f}   (baseline: {p['Baseline Cost Per Ticket']:.2f})")
    lines.append("  Per-class recall (the number that matters most for priority):")
    for label in PRIORITY_LABELS:
        key = f"winner_{label}_recall"
        if key in p:
            lines.append(f"    - {label:<10s}: {p[key]:.1%}")

    lines.append("\n" + "=" * 80)
    lines.append("NOTE: cost matrix charges HIGH->LOW the most (missed urgent ticket),")
    lines.append("while still allowing the model to use the 'low' class in practice.")
    lines.append("=" * 80)

    return "\n".join(lines)


executive_summary_text = format_executive_summary()
...
print("\n[Saved readable summary to executive_summary.txt]")
